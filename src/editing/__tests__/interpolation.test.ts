...
import type { PropDefinition, InterpolatableIdentifier } from '../../types/prop-definition.js'

const str = (name: string): PropDefinition => ({ name, type: { kind: 'primitive', syntax: 'string' }, optional: false })
  ...
  it('expands object params into children', () => {
    const config: PropDefinition = {
      ...
      type: {
        ...
        properties: [str('name'), { name: 'age', type: { kind: 'primitive', syntax: 'number' }, optional: false }],
      },
    }
...
describe('parseInterpolationPath', () => {
  it('parses identifiers and member access', () => {
    ...
    expect(parseInterpolationPath("foo['bar']")).toEqual(['foo', 'bar'])
    expect(parseInterpolationPath('foo["bar"]')).toEqual(['foo', 'bar'])
    expect(parseInterpolationPath("foo['bar'].baz")).toEqual(['foo', 'bar', 'baz'])
    expect(parseInterpolationPath('  foo.bar  ')).toEqual(['foo', 'bar'])
  })
...
describe('resolveInterpolatable', () => {
  ...
  it('resolves known paths and returns the matched node', () => {
    ...
    expect(resolveInterpolatable(['config', 'host'], roots)).toEqual({ name: 'host' })
  })
...
describe('interpolationSuggestions', () => {
  ...
  it('suggests top-level identifiers filtered by the fragment', () => {
    expect(interpolationSuggestions('', roots)).toEqual({ candidates: roots, fragment: '' })
    expect(interpolationSuggestions('ni', roots)).toEqual({ candidates: [{ name: 'nickname' }], fragment: 'ni' })
    expect(interpolationSuggestions('n', roots).candidates.map(c => c.name)).toEqual(['name', 'nickname'])
  })
  ...
  it('filters children by the trailing fragment', () => {
    const { candidates, fragment } = interpolationSuggestions('config.ho', roots)
    expect(candidates.map(c => c.name)).toEqual(['host'])
    expect(fragment).toBe('ho')
  })

  it('drills via bracket accessors', () => {
    expect(interpolationSuggestions("config['", roots).candidates.map(c => c.name)).toEqual(['host', 'port'])
    expect(interpolationSuggestions('config["ho', roots).candidates.map(c => c.name)).toEqual(['host'])
  })
...
describe('collapseTemplateValue', () => {
  it('returns a primitive string when there are no tokens', () => {
    expect(collapseTemplateValue(['hello world'])).toEqual({ kind: 'primitive', value: 'hello world' })
    expect(collapseTemplateValue(['a', 'b'])).toEqual({ kind: 'primitive', value: 'ab' })
    expect(collapseTemplateValue([''])).toEqual({ kind: 'primitive', value: '' })
  })
