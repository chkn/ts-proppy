...
const helper = { callee: 'prompts', import: { name: 'prompts', from: '@evalution/vercel-ai-sdk' } }
...
describe('resolveBindings', () => {
  test('binds through an existing destructure, adding the callee to it', () => {
    const { sourceCode, value } = resolveBindings(destructured, call('anthropic', 'claude'))
    expect(sourceCode).toContain('({ openai, anthropic })')
    expect(value).toEqual({ kind: 'functionCall', callee: 'anthropic', args: [{ kind: 'primitive', value: 'claude' }] })
  })

  test('leaves the destructure alone when the callee is already in it', () => {
    const { sourceCode, value } = resolveBindings(destructured, call('openai', 'gpt-5'))
  ...
  test('adds several callees to the same destructure in one edit', () => {
    const value: PropValue = { kind: 'array', elements: [call('anthropic', 'a'), call('google', 'b')] }
  ...
  test('falls back to the import candidate when the helper is not used', () => {
    ...
    const { sourceCode, value } = resolveBindings(plain, call('openai', 'gpt-4o'))
    expect(sourceCode).toBe(plain)
    expect(value).toMatchObject({ binding: { kind: 'import', spec: { name: 'openai', from: '@ai-sdk/openai' } } })
  })
    ...
    const nested: PropValue = {
      ...
      properties: {
        q: {
          ...
          callee: 'choice',
          args: [call('openai', 'x')],
          binding: [{ kind: 'import', spec: { name: 'choice', from: '@typesafe-ai/sdk' } }],
        },
      },
    }
    const { value } = resolveBindings(`export function p() { return {} }`, nested)
  ...
  test('leaves a single, already resolved binding untouched', () => {
    const bound: PropValue = { kind: 'functionCall', callee: 'openai', args: [], binding: { kind: 'import', spec: { name: 'openai', from: 'x' } } }
