...
export function ensureImport(
  ...
  for (const statement of sourceFile.statements) {
    ...
    if (
      importSpec.isDefault &&
      clause.name &&
      clause.name.text === importSpec.name
    ) {
    ...
    if (
      ...
      const lastElement =
        clause.namedBindings.elements[clause.namedBindings.elements.length - 1];
  ...
  if (lastImportEnd >= 0) {
    return (
      sourceCode.slice(0, lastImportEnd) +
      "\n" +
      importStatement +
      sourceCode.slice(lastImportEnd)
    );
  }
