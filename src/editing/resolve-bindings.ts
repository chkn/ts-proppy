...
export function resolveBindings(
  ...
  const destructureFor = (
    candidate: Extract<CalleeBinding, { kind: "parameter" }>,
  ) => {
    const key = JSON.stringify(candidate.enclosingCall ?? null);
    if (!targets.has(key))
      targets.set(
        key,
        findEnclosingCallDestructure(sourceFile, candidate.enclosingCall),
      );
...
function mapFunctionCalls(
  value: PropValue,
  fn: (call: FunctionCallValue) => FunctionCallValue,
): PropValue {
  switch (value.kind) {
    case "functionCall":
      return fn({
        ...value,
        args: value.args.map(a => mapFunctionCalls(a, fn)),
      });
    case "object":
      return {
        ...value,
