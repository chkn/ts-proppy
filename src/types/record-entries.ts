...
export function recordKeyError(
  ...
  if (next !== current && Object.prototype.hasOwnProperty.call(properties, next)) return 'Key already exists'
  return null
}

/** `base`, or `base2`, `base3`, … — the first that isn't already a key of `properties`. */
export function uniqueRecordKey(properties: Readonly<Record<string, PropValue>>, base = 'key'): string {
...
export function listElements(value: PropValue | undefined): PropValue[] {
  return value?.kind === 'tuple' || value?.kind === 'array' ? value.elements : []
}
