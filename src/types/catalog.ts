  ...
  switch (b.kind) {
    ...
    case "template":
      return (
        ...
        a.value.every((seg, i) => {
          ...
          return typeof seg === "string" || typeof other === "string"
            ? seg === other
            : seg.expr === other.expr;
        })
      );
    ...
    case "object": {
      ...
      return (
        ...
        aKeys.every(
          k =>
            hasOwn(b.properties, k) &&
            valuesEqual(a.properties[k], b.properties[k]),
        )
      );
    }
    case "array":
...
export function findFactory(
  ...
  for (const [catalogIndex, catalog] of catalogs.entries()) {
    for (const [groupIndex, group] of catalog.groups.entries()) {
      if (group.factory?.def.name === value.callee)
        return { catalogIndex, groupIndex, factory: group.factory };
    }
  }
  return undefined;
}
...
export function literalDefinition(
  ...
  return {
    ...catalog.literal,
    name: catalog.literal.name || slot.name,
    interpolatables: slot.interpolatables,
  };
}
  ...
  switch (type.kind) {
    ...
    case "primitive": {
      ...
      if (base === "string" || base === "number" || base === "boolean")
        return typeof value.value === base;
      return true;
    }
    case "union":
    ...
    case "object": {
      ...
      return (
        ...
        type.properties.every(p =>
          hasOwn(value.properties, p.name)
            ? valueFitsType(p.type, value.properties[p.name])
            : p.optional,
        )
      );
    }
 ...
 */
export function activeCatalogIndex(
  catalogs: readonly ValueCatalog[],
  slot: PropDefinition,
  value: PropValue | undefined,
): number {
...
function parametersOf(factory: ValueFactory): PropDefinition[] {
  return factory.def.type.kind === "function"
    ? factory.def.type.parameters
    : [];
}
...
export function defaultCall(factory: ValueFactory): FunctionCallValue {
  ...
  return {
    ...
    args: params
      .slice(0, count)
      .map(p => p.defaultValue ?? defaultValueForType(p.type)),
...
export function setCallArgument(
  ...
  while (args.length > 0) {
    ...
    const a = args[last];
    if (p?.optional && a.kind === "primitive" && a.value === undefined)
      args.pop();
    else break;
  }
  return { ...call, args };
