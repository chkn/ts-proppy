...
describe("renameRecordKey", () => {
  ...
  test("leaves the entries unchanged for a key that does not exist", () => {
    expect(Object.keys(renameRecordKey({ a: v("1") }, "nope", "z"))).toEqual([
      "a",
    ]);
  });
});
...
describe("uniqueRecordKey", () => {
  test("numbers the base until it is free", () => {
    ...
    expect(
      uniqueRecordKey({ question: v(""), question2: v("") }, "question"),
    ).toBe("question3");
  });
});
...
describe("open string unions", () => {
  const union = (...types: PropType[]): PropType => ({
    kind: "union",
    syntax: "",
    types,
  });
  const constant = (value: unknown): PropType => ({
    kind: "constant",
  ...
  test("accepts a template literal or branded string as the open member", () => {
    const branded: PropType = {
      kind: "primitive",
      syntax: "string & {}",
      base: "string",
    };
    expect(getOpenStringUnionInfo(union(constant("a"), branded))).toEqual({
      suggestions: ["a"],
    });
  });
  ...
  test("rejects a union with any member that is not a string", () => {
    ...
    expect(
      getOpenStringUnionInfo(union(constant("a"), str, constant(null))),
    ).toBeNull();
...
describe("defaultValueForType", () => {
  ...
  test("fills a tuple to its fixed length", () => {
    const str = {
      name: "",
      optional: false,
      type: { kind: "primitive", syntax: "string" } as PropType,
    };
    const type: PropType = {
      kind: "tuple",
