...
const openai: ValueFactory = {
  def: def("openai", {
    kind: "function",
    syntax: "(id: string) => LanguageModel",
    parameters: [def("modelId", str)],
  }),
  binding: [
  ...
  groups: [
    {
      label: "OpenAI",
      icon: "openai",
      presets: [{ label: "GPT-5.5", value: gpt("gpt-5.5", openai.binding) }],
      factory: openai,
    },
  ],
};
const gatewayCatalog: ValueCatalog = {
  ...
  groups: [
    {
      label: "OpenAI",
      presets: [{ label: "GPT-5.5", value: text("openai/gpt-5.5") }],
    },
  ],
  literal: true,
};
const catalogs = [providerCatalog, gatewayCatalog];
const slot = def("model", {
  kind: "union",
  syntax: "LanguageModel",
  types: [str, { kind: "opaque", syntax: "LanguageModelV3" }],
});

describe("preset selection", () => {
  test("finds a preset equal to the value, ignoring its binding candidates", () => {
    const inFile = gpt("gpt-5.5", { kind: "parameter" });
    expect(findPreset(catalogs, inFile)).toMatchObject({
      catalogIndex: 0,
      groupIndex: 0,
      preset: { label: "GPT-5.5" },
    });
    expect(findPreset(catalogs, text("openai/gpt-5.5"))).toMatchObject({
      catalogIndex: 1,
    });
  });
  ...
  test("compares templates, arrays and objects structurally", () => {
    expect(
      valuesEqual(
        { kind: "template", value: ["a", { expr: "b" }, ""] },
        { kind: "template", value: ["a", { expr: "b" }, ""] },
      ),
    ).toBe(true);
    expect(
      valuesEqual(
        { kind: "array", elements: [text("a")] },
        { kind: "tuple", elements: [text("a")] },
      ),
    ).toBe(true);
    expect(
      valuesEqual(
        { kind: "object", properties: { a: text("1") } },
        { kind: "object", properties: { a: text("2") } },
      ),
    ).toBe(false);
  });
});
...
describe("factory calls", () => {
  ...
  test("builds a default call with the factory binding and required arguments", () => {
    expect(defaultCall(openai)).toEqual({
      kind: "functionCall",
      callee: "openai",
      args: [text("")],
      binding: openai.binding,
    });
  });
  ...
  test("gives a factory with only optional parameters its first argument", () => {
    const noul: ValueFactory = {
      def: def("noul", {
        ...
        syntax: "",
        parameters: [
          def("instructions", str, true),
          def("criteria", str, true),
        ],
      }),
      binding: {
        kind: "import",
        spec: { name: "noul", from: "@typesafe-ai/sdk" },
      },
    };
  ...
  test("edits one argument, padding skipped optional ones and trimming trailing ones", () => {
    ...
    const call: Extract<PropValue, { kind: "functionCall" }> = {
      kind: "functionCall",
      callee: "f",
      args: [],
    };
    const withC = setCallArgument(call, params, 2, text("c"));
    expect(withC.args).toEqual([
      { kind: "primitive", value: undefined },
      { kind: "primitive", value: undefined },
      text("c"),
    ]);
    const cleared = setCallArgument(withC, params, 2, {
      kind: "primitive",
      value: undefined,
    });
  ...
  test("pads a skipped required argument with its default", () => {
    ...
    expect(
      setCallArgument(
        { kind: "functionCall", callee: "f", args: [] },
        params,
        1,
        text("b"),
      ).args,
...
describe("narrowed literals", () => {
  ...
  const modelMember: PropType = {
    kind: "object",
    syntax: "{ model: Model }",
    properties: [def("model", str)],
  };
  const agentMember: PropType = {
    kind: "object",
    syntax: "{ agent: Agent }",
    properties: [def("agent", str)],
  };
  const slot = def("model", {
    kind: "union",
    syntax: "",
    types: [modelMember, agentMember],
  });
  const models: ValueCatalog = {
    label: "Models",
    groups: [],
    literal: def("", modelMember),
  };
  const agents: ValueCatalog = {
    label: "Agents",
    groups: [],
    literal: def("", agentMember),
  };
  ...
  test("narrows each catalog's free-form entry to its own member", () => {
    ...
    expect(
      literalDefinition(
        { label: "x", groups: [], literal: true },
        { ...slot, catalogs: [models] },
      ),
    ).toMatchObject({ catalogs: undefined });
  });

  test("opens a value in the catalog whose member it fits", () => {
    const agent: PropValue = {
      kind: "object",
      properties: { agent: text("deep-research") },
    };
    const model: PropValue = {
      kind: "object",
      properties: { model: text("gemini-3.5-flash") },
    };
  ...
  test("does not fit an object with a key the member lacks", () => {
    expect(
      valueFitsType(modelMember, {
        kind: "object",
        properties: { model: text("x"), agent: text("y") },
      }),
    ).toBe(false);
  });
});
