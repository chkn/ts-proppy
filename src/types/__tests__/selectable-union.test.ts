...
const arr: PropType = {
  kind: "array",
  syntax: "string[]",
  element: { name: "", type: str, optional: false },
};
...
describe("matchUnionMember", () => {
  test("finds the constant a value holds", () => {
    ...
    expect(matchUnionMember(info, { kind: "primitive", value: "auto" })).toBe(
      0,
    );
    expect(matchUnionMember(info, { kind: "primitive", value: "none" })).toBe(
      1,
    );
  });
  ...
  test("prefers an exact constant over a member that merely accepts the shape", () => {
    ...
    expect(matchUnionMember(info, { kind: "primitive", value: "other" })).toBe(
      0,
    );
  });
  ...
  test("falls back to the default member when no member fits", () => {
    ...
    expect(
      matchUnionMember(info, { kind: "primitive", value: "unlisted" }),
    ).toBe(1);
  });
  ...
  test("distinguishes null from undefined and from an empty string", () => {
    ...
    expect(
      matchUnionMember(info, { kind: "primitive", value: undefined }),
    ).toBe(2);
