...
export function defaultValueForType(type: PropType): PropValue {
  switch (type.kind) {
    ...
    case "tuple":
      return {
        kind: "array",
        elements: type.elements.map(e => defaultValueForType(e.type)),
      };
    case "object":
      return {
        kind: "object",
    ...
    case "union": {
      ...
      const empty = type.types.find(
        t =>
          t.kind === "constant" && (t.value === null || t.value === undefined),
      );
      if (empty) return defaultValueForType(empty);
      return type.types.length > 0
        ? defaultValueForType(type.types[0])
