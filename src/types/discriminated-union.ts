...
export function getDiscriminatedUnionInfo(
  ...
  const objectTypes = propType.types.filter(
    (t): t is Extract<PropType, { kind: "object" }> => t.kind === "object",
  );
  ...
  for (const prop of firstProps) {
    ...
    if (isDiscriminator) {
      const cases: DiscriminatedUnionCase[] = objectTypes.map(obj => {
        const discriminatorProp = obj.properties.find(
          p => p.name === prop.name,
        )!;
        return {
          discriminatorValue: (
            discriminatorProp.type as Extract<PropType, { kind: "constant" }>
          ).value,
