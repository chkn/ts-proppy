...
import {
  ...
  useInterpolatables,
} from "./interpolatables-context.js";
...
export function ItemEditor(props: ItemEditorInternalProps) {
  ...
  return own && own !== inherited ? (
    <InterpolatablesContext.Provider value={own}>
      {editor}
...
function offersReference(type: PropType): boolean {
  switch (type.kind) {
    ...
    case "union":
      ...
      return (
        !type.types.every(t => t.kind === "constant") &&
        !type.types.some(t => primitiveBase(t) === "string")
      );
    case "primitive":
      return (
        !["string", "number", "boolean"].includes(primitiveBase(type) ?? "") &&
        type.syntax !== "Date"
      );
    default:
      return false;
...
function RoutedEditor(props: ItemEditorInternalProps) {
  ...
  if (value?.kind === "reference") {
    return (
      <ReferenceEditor
        ...
        onChange={onChange}
        onClear={() =>
          onChange(propDef.defaultValue ?? defaultValueForType(propDef.type))
        }
        disabled={disabled}
      />
    );
  ...
  if (
    !interpolatables?.length ||
    propDef.catalogs?.length ||
    !offersReference(propDef.type)
  )
    return editor;
  return (
...
function SlotEditor({
  ...
  if (
    value?.kind === "template" ||
    (primitiveBase(type) === "string" && propDef.interpolatables?.length)
  ) {
    return (
      <TemplateEditor
        propDef={propDef}
