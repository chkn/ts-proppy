...
function absOffsetOf(
  ...
  while (current) {
    if (current === node)
      return offset + (current.nodeType === Node.TEXT_NODE ? offsetInNode : 0);
    if (current.nodeType === Node.TEXT_NODE)
      offset += current.textContent?.length ?? 0;
    else if (current instanceof HTMLElement && current.tagName === "BR")
      offset += 1;
...
function getActiveInterpolation(root: HTMLElement): ActiveInterpolation | null {
  ...
  while (p && p !== root) {
    if (p instanceof HTMLElement && p.classList.contains("te-token"))
      return null;
    p = p.parentNode;
  }
  ...
  return {
    ...
    dollarOffset,
    caret,
    activeExpr: before.slice(dollarOffset + 2),
  };
}

function caretRect(root: HTMLElement): {
  left: number;
  top: number;
  bottom: number;
} {
...
function itemsEqual(a: SuggestItem[], b: SuggestItem[]): boolean {
  ...
  return a.every((item, i) => {
    ...
    if (item.kind === "identifier" && other.kind === "identifier")
      return item.node.name === other.node.name;
    return true;
  });
}
...
export function TemplateEditor({
  ...
  const renderValue = (
    nextValue: TemplateValue,
    preserveSelection: boolean,
  ) => {
    if (!editorRef.current) return;
    const selectionOffset = preserveSelection
      ? getTextOffset(editorRef.current)
      : null;
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: seeds the contentEditable once on mount; later syncs go through the effect below.
  useLayoutEffect(() => {
    renderValue(tmplValue, false);
  }, []);

  // biome-ignore lint/correctness/useExhaustiveDependencies: renderValue is re-created every render; only an actual external value change may clobber the DOM the user is typing into.
  useEffect(() => {
  ...
  const updateSuggestions = () => {
    ...
    const items: SuggestItem[] = candidates.length
      ? [
          ...candidates.map(
            node => ({ kind: "identifier", node }) as SuggestItem,
          ),
          { kind: "insert-literal" },
        ]
    ...
    setSuggest(prev => {
      ...
      const index =
        prev && itemsEqual(prev.items, items)
  ...
  const commitActive = (trailing = ""): boolean => {
    ...
    if (
      dismissedDollar.current ===
      absOffsetOf(root, active.node, active.dollarOffset)
    )
      return false;
  ...
  const acceptIdentifier = (
    ...
    if (selection?.rangeCount) {
      ...
      r.setStart(
        ...
        Math.max(0, range.startOffset - fragmentLen),
      );
  ...
  const acceptItem = (
    ...
    if (item.kind === "identifier") {
      ...
      const root = editorRef.current!;
      const active = getActiveInterpolation(root);
      dismissedDollar.current = active
        ? absOffsetOf(root, active.node, active.dollarOffset)
        : null;
      accepted.current = false;
      setSuggest(null);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (suggest) {
      ...
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setSuggest({ ...suggest, index: (suggest.index + 1) % n });
        return;
      }
      if (e.key === "ArrowUp") {
        e.preventDefault();
        setSuggest({ ...suggest, index: (suggest.index - 1 + n) % n });
        return;
      }
      if (e.key === "Escape") {
        e.preventDefault();
      ...
      if (e.key === "Enter") {
        e.preventDefault();
        acceptItem(suggest.items[suggest.index], suggest.fragmentLen, true);
        return;
      }
      if (e.key === "Tab") {
        e.preventDefault();
        acceptItem(suggest.items[suggest.index], suggest.fragmentLen, false);
        return;
      }
    }
  ...
  return (
    <>
      {/* biome-ignore lint/a11y/noStaticElementInteractions: a contentEditable surface is the interactive element here; no semantic tag covers it. */}
      <div
        ref={editorRef}
          ...
          {suggest.items.map((item, i) => {
            ...
            return (
              <li
                ...
                className={
                  "te-suggest-item" +
                  (i === suggest.index ? " active" : "") +
                  (action ? " te-suggest-action" : "")
                }
                onMouseDown={e => {
                  e.preventDefault();
                  // Clicking a parent (has children) drills in; a leaf commits.
                  const close = !(
                    item.kind === "identifier" && !!item.node.children?.length
                  );
                  acceptItem(item, suggest.fragmentLen, close);
                }}
                onMouseEnter={() =>
                  setSuggest(s => (s ? { ...s, index: i } : s))
                }
                style={{
                  padding: "4px 8px",
                  borderRadius: 4,
                  ...
                  borderTop:
                    action &&
                    i > 0 &&
                    suggest.items[i - 1].kind === "identifier"
                      ? `1px solid ${colors.border}`
                      : undefined,
                  background:
                    i === suggest.index ? colors.menuActiveBg : "transparent",
                }}
              >
                {label}
              </li>
