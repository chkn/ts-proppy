    ...
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {paths.length > 0 && (
        <select
          ...
          disabled={disabled}
          onChange={e =>
            onChange({ kind: "reference", path: e.target.value.split(".") })
          }
          style={{ ...controlStyle, width: "auto", flex: "0 1 auto" }}
        >
          {!paths.some(p => p.join(".") === current) && (
            <option value={current}>{referenceToSource(path)}</option>
          )}
          {paths.map(p => (
...
export function ParameterMenu({
  ...
  return (
    ...
    >
      {/* biome-ignore lint/suspicious/noTemplateCurlyInString: placeholder text that deliberately shows interpolation syntax to the user. */}
      <option value="">{"${…}"}</option>
      {paths.map(p => (
        <option key={p.join(".")} value={p.join(".")}>
          {referenceToSource(p)}
