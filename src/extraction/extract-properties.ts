...
export function extractPropertiesFromTypeNode(
  ...
  const definitions = extractDefinitionsFromTypeNode(
    typeNode,
    sourceFile,
    typeChecker,
  );
...
export function extractDefinitionsFromTypeNode(
  ...
  if (ts.isTypeLiteralNode(typeNode)) {
    ...
    for (const member of typeNode.members) {
      if (ts.isPropertySignature(member) && member.name) {
        props.push(
          extractPropertyFromSignature(member, sourceFile, typeChecker),
        );
      }
    }
    return props;
  ...
  if (ts.isTypeReferenceNode(typeNode)) {
    ...
    if (typeDecl) {
      return extractDefinitionsFromDeclaration(
        typeDecl,
        declSourceFile,
        typeChecker,
      );
  ...
  if (ts.isFunctionTypeNode(typeNode)) {
    ...
    ts.isFunctionTypeNode(typeNode.type)
  ) {
    ...
    for (const unionMember of typeNode.types) {
      if (ts.isFunctionTypeNode(unionMember)) {
        ...
        ts.isParenthesizedTypeNode(unionMember) &&
        ts.isFunctionTypeNode(unionMember.type)
      ) {
...
export function extractPropertiesFromParameters(
  ...
  const definitions = extractDefinitionsFromParameters(
    parameters,
    sourceFile,
    typeChecker,
  );
  ...
  for (const param of parameters) {
    if (ts.isIdentifier(param.name)) {
      ...
      if (param.initializer) {
        def.defaultValue = parseValueFromExpression(
          param.initializer,
          sourceFile,
        );
      }
...
export function extractPropertiesFromDeclaration(
  ...
  const definitions = extractDefinitionsFromDeclaration(
    decl,
    sourceFile,
    typeChecker,
  );
      ...
      for (const heritageClause of decl.heritageClauses) {
        if (heritageClause.token === ts.SyntaxKind.ExtendsKeyword) {
          for (const typeExpr of heritageClause.types) {
            ...
            if (baseDecl) {
              const inheritedProps = extractDefinitionsFromDeclaration(
                baseDecl,
                baseSrcFile,
                typeChecker,
              );
