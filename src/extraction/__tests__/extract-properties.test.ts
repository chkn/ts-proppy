...
function extractFromSource(source: string) {
  ...
  function visit(node: ts.Node) {
    ...
    if (ts.isExportAssignment(node) && !node.isExportEquals) {
      ...
      if (ts.isArrowFunction(expr) && expr.parameters.length > 0 && expr.parameters[0].type) {
...
`)
    ...
    expect(result.definitions[0]).toEqual({ name: 'name', type: { kind: 'primitive', syntax: 'string' }, optional: false })
    expect(result.definitions[1]).toEqual({ name: 'age', type: { kind: 'primitive', syntax: 'number' }, optional: false })
    expect(result.definitions[2]).toEqual({ name: 'optional', type: { kind: 'primitive', syntax: 'boolean' }, optional: true })
  })
...
`)
    ...
    expect(result.definitions[0]).toEqual({ name: 'title', type: { kind: 'primitive', syntax: 'string' }, optional: false })
    expect(result.definitions[1]).toEqual({ name: 'count', type: { kind: 'primitive', syntax: 'number' }, optional: true })
  })

  test('returns empty definitions when no params', () => {
    const sourceFile = ts.createSourceFile('test.tsx', 'const x = 1', ts.ScriptTarget.Latest, true)
...
`)
    ...
    expect(result.definitions[0].description).toBe('Second comment (should be used)')
  })
...
`)
    ...
    if (result.definitions[1].type.kind === 'function') {
      expect(result.definitions[1].type.parameters[0].name).toBe('x')
      expect(result.definitions[1].type.parameters[0].type.syntax).toBe('number')
    }
  })
...
`)
    ...
    if (result.definitions[0].type.kind === 'object') {
      ...
      expect(result.definitions[0].type.properties[0].description).toBe('API key')
      expect(result.definitions[0].type.properties[1].name).toBe('timeout')
      expect(result.definitions[0].type.properties[1].description).toBe('Timeout in milliseconds')
    }
  })
