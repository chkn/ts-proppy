...
function paramTypes(source: string, { checker }: { checker: boolean }): PropType[] {
  ...
  if (checker) {
    const host: ts.CompilerHost = {
      getSourceFile: (name, languageVersion) => {
        const text = name === entry ? source : name.startsWith(LIB_DIR) ? ts.sys.readFile(name) : undefined
    ...
    sourceFile = ts.createSourceFile(entry, source, ts.ScriptTarget.Latest, true)
  }
  ...
  visit(sourceFile)
  return extractDefinitionsFromParameters(fn!.parameters, sourceFile, typeChecker).map(d => d.type)
}
...
describe.each(modes)('records (%s)', (_, opts) => {
  test.each([
    ...
    ['an index-signature literal', 'function f(x: { [name: string]: number }) {}'],
    ['an index-signature interface', 'interface Scores { [name: string]: number }\nfunction f(x: Scores) {}'],
  ])('builds a record from %s', (_, source) => {
    const [type] = paramTypes(source, opts)
    expect(type.kind).toBe('record')
    if (type.kind !== 'record') return
    expect(type.value).toMatchObject({ name: '', optional: false, type: { kind: 'primitive' } })
    expect(type.value.type.syntax).toBe('number')
  })

  test('keeps an object with named members an object', () => {
    const [type] = paramTypes('function f(x: { a: string; [name: string]: string }) {}', opts)
...
describe.each(modes)('variadic tuples (%s)', (_, opts) => {
  test('reads the rest element of [A, A, ...A[]]', () => {
    const [type] = paramTypes('type E = string | null\nfunction f(x: readonly [E, E, ...E[]]) {}', opts)
  ...
  test('marks optional elements optional', () => {
    const [type] = paramTypes('function f(x: [string, number?]) {}', opts)
    if (type.kind !== 'tuple') throw new Error(`expected a tuple, got ${type.kind}`)
...
describe.each(modes)('open string unions (%s)', (_, opts) => {
  test("recognizes 'a' | 'b' | (string & {})", () => {
    const [type] = paramTypes("function f(model: 'a' | 'b' | (string & {})) {}", opts)
...
function f(factory: typeof noul) {}`
    ...
    expect(type.parameters[0].type).toMatchObject({ kind: 'primitive', syntax: 'string' })
  })
})
