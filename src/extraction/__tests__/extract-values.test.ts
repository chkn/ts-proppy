...
function extractValues(
  source: string,
  definitions: PropDefinition[] | undefined = undefined,
) {
  ...
  return extractPropertiesFromObjectLiteral(
    findObjectLiteral(sourceFile),
    definitions,
    sourceFile,
  );
}
...
describe("extractPropertiesFromObjectLiteral", () => {
  ...
  test("adds valueSpan to definitions", () => {
    ...
    expect(source.slice(nameDef.valueSpan!.start, nameDef.valueSpan!.end)).toBe(
      '"Alice"',
    );
  });
  ...
  test("extracts nested object values", () => {
    ...
    const defs: PropDefinition[] = [
      {
        ...
        type: {
          kind: "object",
          syntax: "{ key: string; timeout: number }",
          properties: [],
        },
        optional: false,
      },
    ...
    if (result.values!.config.kind === "object") {
      expect(result.values!.config.properties.key).toEqual({
        kind: "primitive",
        value: "abc",
      });
      expect(result.values!.config.properties.timeout).toEqual({
        kind: "primitive",
        value: 5000,
      });
    }
  ...
  test("extracts array values", () => {
    ...
    if (result.values!.items.kind === "array") {
      ...
      expect(result.values!.items.elements[0]).toEqual({
        kind: "primitive",
        value: "a",
      });
    }
  ...
  test("extracts function call values", () => {
    ...
    if (result.values!.model.kind === "functionCall") {
      ...
      expect(result.values!.model.binding).toEqual({
        kind: "import",
        spec: { name: "openai", from: "ai" },
      });
    }
  });
  ...
  test("extracts arrow function values", () => {
    ...
    const defs: PropDefinition[] = [
      {
        name: "onClick",
        type: { kind: "function", syntax: "(e: any) => void", parameters: [] },
        optional: false,
      },
    ];
    const result = extractValues(source, defs);
...
describe("extractPropertiesFromObjectLiteral (schemaless)", () => {
  ...
  test("infers array type from first element", () => {
    ...
    expect(result.definitions[0].type).toMatchObject({
      kind: "array",
      element: { type: { kind: "primitive", syntax: "string" } },
    });
  });
  ...
  test("infers object type from properties", () => {
    ...
    if (configType.kind === "object") {
      ...
      expect(configType.properties[1]).toMatchObject({
        name: "timeout",
        type: { kind: "primitive", syntax: "number" },
      });
    }
...
describe("parseValueFromExpression – string concatenation", () => {
  ...
  test("produces a template when an identifier is mixed in", () => {
    ...
    expect(result.values!.greeting).toEqual({
      kind: "template",
  ...
  test("produces a template for identifier + literal", () => {
    ...
    expect(result.values!.path).toEqual({
      kind: "template",
      value: ["", { expr: "base" }, "/suffix"],
    });
  });
  ...
  test("appends a literal after a template expression", () => {
    ...
    expect(result.values!.msg).toEqual({
      kind: "template",
      value: ["Hello ", { expr: "name" }, "!"],
    });
  });
  ...
  test("prepends a literal before a template expression", () => {
    const result = extractValues(
      'const x = { msg: "prefix: " + `${value} end` }',
    );
    expect(result.values!.msg).toEqual({
      kind: "template",
      value: ["prefix: ", { expr: "value" }, " end"],
    });
  });
